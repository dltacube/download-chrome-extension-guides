...
function extractIndex(url, doc) {
    ...
    if (doc.find('.inline-site-toc').length) {
        var t = doc.find('h1').text();
        if (doc.find('.inline-site-toc').length)
            index[url][1] = 'Guide';
    };
}
    ...
    function () {
        ...
        getHTML(p, function (url, doc) {
            ...
            if (url.match(/api_index$/)) {
                ...
                sidebar = $(body.find("li[class='show']")[0]);
                sidebar.find('a').each(function () {
                    if ($(this).attr('href').match(/https:\/\/developer\.chrome\.com\/extensions/)) {
                        var h = $(this).attr('href');
                        addPage(h)
                    }
                });
